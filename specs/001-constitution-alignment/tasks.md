...
- [X] T001 Verify current branch is `001-constitution-alignment` and working tree is clean
...
- [X] T002 [US1] Create `.gitignore` at repository root with entries: `config.json`, `*.secret*`, `.env`, `__pycache__/`, `*.pyc`, `output/`, `log/`
- [X] T003 [US1] Verify that existing untracked `__pycache__/` directory is now ignored by running `git status`
- [X] T004 [US1] Test secret protection by creating a temporary `config.json` and `.env`, confirming `git status` does not show them, then removing the test files
...
- [X] T005 [US2] Create directory `.github/workflows/` at repository root
- [X] T006 [US2] Create `.github/workflows/ci.yml` with workflow trigger on `pull_request` to `main` and `push` to `main`
- [X] T007 [US2] Add lint job to `.github/workflows/ci.yml`: checkout repo, install `ruff` via pip, run `ruff check convert.py`
- [X] T008 [US2] Add FHIR validation job to `.github/workflows/ci.yml`: checkout repo, setup Java 17, download/cache `validator_cli.jar` from `https://github.com/hapifhir/org.hl7.fhir.core/releases/latest/download/validator_cli.jar`, run `python3 convert.py --config config.example.json` against each CSV in `input/` matching `*.BedCapacity.csv` (excluding `*column-labels-only*`), then validate all output JSON Bundles with `java -jar validator_cli.jar output/**/*.json -version 4.0.1 -ig hl7.fhir.us.safr`
- [X] T009 [US2] Add secret scanning job to `.github/workflows/ci.yml`: use `gitleaks/gitleaks-action@v2` to scan for committed credentials
- [X] T010 [P] [US2] Create `ruff.toml` at repository root with minimal configuration (line-length = 120 or match existing convert.py style)
- [X] T011 [US2] Run `ruff check convert.py` locally and fix any lint violations in `convert.py` (minor style fixes only — no functional changes)
...
- [X] T012 [US3] Update `config.example.json` server section: replace `"base_url": ""` with `"base_url": "YOUR_FHIR_SERVER_URL"`, `"token_endpoint": ""` with `"token_endpoint": "YOUR_TOKEN_ENDPOINT"`, `"client_id": ""` with `"client_id": "YOUR_CLIENT_ID"`, `"client_secret": ""` with `"client_secret": "YOUR_CLIENT_SECRET"`
...
- [X] T013 [US4] Verify that the CI workflow in `.github/workflows/ci.yml` (from T008) correctly globs `input/*.BedCapacity.csv` and excludes `*column-labels-only*` files
- [X] T014 [US4] Add a comment in `.github/workflows/ci.yml` documenting how to add new test fixtures (place CSV in `input/` with `*.BedCapacity.csv` naming convention)
...
- [X] T015 Run `ruff check convert.py` to confirm lint passes
- [X] T016 Run `python3 convert.py input/2025.10.21.Test.Facility.BedCapacity.csv --config config.example.json` to confirm converter still works with updated config placeholders (expect graceful handling of placeholder values when no `--fhir-server` is used)
- [X] T017 Review all new/modified files for consistency with constitution principles
- [X] T018 Run quickstart.md validation steps locally
