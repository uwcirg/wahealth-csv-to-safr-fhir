...
- [X] T007 [P] [US2] In `tests/test_output_layout.py`: add a test asserting that without `--bundles-mrs-only` the full set of four individual resources is still written (regression guard for the default), and a test asserting `convert.py --help` output (capture via subprocess) contains the string `--bundles-mrs-only`.
...
- [X] T010 [US2] Run `tests/test_output_layout.py` (T006, T007, T007a) plus the full `pytest` suite and `ruff check .`; confirm pass.
